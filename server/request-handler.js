...
exports.requestHandler = function(request, response) {
  ...
  // var read = fs.readFileSync('chats.txt'); //we were storing everything in a text file, but now we need to redirect to our database
  ...
  //this is where we starting writing to the text file
  ...
  if (request.method === 'GET'){
    ...
    if(request.url === '/') {
      ...
      var room = request.json.roomname || '*'
      //query 'select from ' messages and whatnot
      connection.query('SELECT * FROM messages WHERE roomname = '+room, function(err, result){//access roomname
        if (err) throw err; //need to join in our query
        else {
          //retrieve messages
